...
{
    // Convert visio:// to visio:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("visio://"))
    {
        uri.replace(0, 12, "visio:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Visio.lnk";
}
...
{
    return GetAutostartDir() / "visio.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Visio-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  visio-qt [" + tr("command-line options") + "]                     " + "\n";
